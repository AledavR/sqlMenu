...
public class coneccionSQL {
    ...
    public static final String URL="jdbc:mysql://localhost:3306/personalmedico?autoReconnet=true&useSSL=false";
	...
	protected void imprimirFilas(ResultSet rs){
        ...
        protected void imprimirFilasPacientes(ResultSet rs){
		try {
			imprimirHeaders();
			while(rs.next())
				{
					System.out.printf("%-4s %-12s %-22s %-22s %-12s %-12s %-12s \n",
									  rs.getString("ID"),
									  rs.getString("ApellidoPat"),
